...
## Updating after you've already deployed

If you already deployed and tested before adding a new field (like the comment link), two things need doing:

1. **Re-deploy the script**: go back into Apps Script, paste in the updated `apps-script.gs`, then **Deploy → Manage deployments → click the pencil/edit icon → Version: New version → Deploy**. The `/exec` URL stays the same, so you don't need to update `index.html` again.
2. **Fix the header row**: the script only writes headers automatically on a completely empty sheet. If you already have a header row from before, either delete it (row 1) so it regenerates with the new "Comment Link" column, or manually add a "Comment Link" header yourself in the right position.
